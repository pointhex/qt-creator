...
public:
    ...
    {
        const QModelIndex sourceIndex = m_proxyModel->mapFromSource(m_sourceModel->index(row, 0));
        if (sourceIndex.isValid()) {
            m_engineChooser->setCurrentIndex(sourceIndex.row());
...
void EngineManagerPrivate::activateEngineByIndex(int index)
{
    ...
    qDebug() << "activateEngineByIndex" << index << engineItem << engineItem->m_engine;
    if (engineItem->m_engine)
        qDebug() << engineItem->m_engine->debuggerType();
